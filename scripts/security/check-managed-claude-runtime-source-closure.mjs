...
while (pending.length) {
  ...
  if (relativePath !== trustedEntrypoint) discovered.add(relativePath);
  for (const edge of localImportEdges(source)) {
    const imported = normalizeProjectPath(resolve(
      dirname(resolve(packageRoot, relativePath)),
      edge,
    ));
    if ([trustedEntrypoint, trustedBootstrap].includes(imported)) continue;
...
if (process.argv.includes('--actual-json')) {
  process.stdout.write(`${JSON.stringify(actual, null, 2)}\n`);
  process.exit(0);
}
const expectedKeys = Object.keys(MANAGED_CLAUDE_RUNTIME_SOURCE_SHA256).sort();
const actualKeys = Object.keys(actual).sort();
if (JSON.stringify(expectedKeys) !== JSON.stringify(actualKeys)) {
  fail(`source graph mismatch\nexpected: ${expectedKeys.join(', ')}\nactual:   ${actualKeys.join(', ')}`);
}
for (const path of actualKeys) {
  if (MANAGED_CLAUDE_RUNTIME_SOURCE_SHA256[path] !== actual[path]) {
    fail(`source digest mismatch: ${path}\nexpected ${MANAGED_CLAUDE_RUNTIME_SOURCE_SHA256[path]}\nactual   ${actual[path]}`);
  }
}
process.stdout.write(
  `Managed Claude runtime source closure verified (${actualKeys.length} files)\n`,
);
...
function fail(message) {
  process.stderr.write(
    `Managed Claude runtime source closure check failed: ${message}\n`,
  );
  process.exit(1);
}
