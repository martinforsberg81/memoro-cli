...
mc plan     → a foreground session for one programme writes one
              docs/project/<programme>/<project>/PLAN.json per project
...
- **You do not write plan state by hand.** `mc plan <programme>` opens the
  session that writes them, with Martin in it — one plan per project the
  programme can start now. A step session edits its own step's `status` and
