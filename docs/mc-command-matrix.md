...
| `mc repo merge <repo> <pr>` | Run the test gate and, only if nothing new went red, squash-merge, deploy-pull, freshen the open branches the merge made dirty (base merged in, plain push, owner's inbox told; conflicts touch nothing) and log it. `--check` gates and stops. `--json`. |
