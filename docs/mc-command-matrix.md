...
| Command | Does |
|---|---|
| `mc` | The one page — NEXT, INTAKE, PROGRAMMES, WORK, RUNNER, HELPER, BRIEF — and at a terminal the way in under it: a number opens that project's workarea, making one if it has none, `n` starts something new, `b`/`p <name>`/`s <name>` run brief, plan and one project, `q` quits, and any other line is read as an `mc work` command. At a terminal the page is live: every 30 s the rows that changed are rewritten where they stand, the prompt and a half-typed answer untouched (see [`docs/technical/mc-ui.md`](technical/mc-ui.md)). Piped it prints once, exits 0 and writes no escape byte. PROGRAMMES is one row per `PLAN.json` on `origin/main`, under its programme's own heading, with the workareas no project explains under WORK; NEXT is the order `mc run` would take, one block per lane. `--json`, `--fresh`. |
