...
| `mc brief <no args>` | The evaluation session: collect what the runner merged, opened and is waiting on into `~/mc/brief/<date>.md`, then open a fresh session on it. `--collect` writes only the file — no model — and `--offline` keeps it off the network. |
