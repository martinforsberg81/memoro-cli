...
| `mc test nightly start \| stop \| status` | The same full round on an interval, with nobody asking: every repository mc knows, once a day by default (`--interval <seconds>` on start), written to `~/.memoro/mc/nightly/nightly.log`. A meter — what it finds refuses no merge and delays no round — and a tick that meets a running gate round skips it rather than queueing. `status` says whether it is running, how often, where it writes, and per repository what the last measured run found: when, the commit, `N red of M`, and the oldest red test with the date its streak began; `--json` carries `running`, `pid`, `interval_ms`, `log` and a `repos` object of one reading each. See [`docs/technical/mc-merge.md`](technical/mc-merge.md). |
