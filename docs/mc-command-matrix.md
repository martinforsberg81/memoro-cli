...
**Status:** current · 2026-09-03 · derived from `src/mc-cli.js` and `src/bin-mc.js`
...
is not listed here it does not exist. Nothing is listed as *being removed* any
more: `src/mc-cli.js` routes twelve verbs and `src/bin-mc.js` routes one, and
this file describes those thirteen and the page above them.
...
| Command | Does |
|---|---|
| `mc` | The one page — PROGRAMMES, WORK, HELPER, BRIEF, NEXT, RUNNER, MERGES, DEPLOY and a line for mc itself — and at a terminal the way in under it: a number opens that project's workarea, making one if it has none, `n` starts something new, `b`/`p <name>`/`s <name>` run brief, plan and one project, `q` quits, and any other line is read as an `mc work` command. At a terminal the page is live: every 30 s the rows that changed are rewritten where they stand, the prompt and a half-typed answer untouched (see [`docs/technical/mc-ui.md`](technical/mc-ui.md)). Piped it prints once, exits 0 and writes no escape byte. PROGRAMMES is one row per `PLAN.json` on `origin/main`, under its programme's own heading, with the workareas no project explains under WORK; NEXT is the order `mc run` would take, as one list of at most four per repository. `--json`, `--fresh`. |
...
| `mc roles check [<role>]` | What a launch would hand a session for that role **today** — profile, `_common.md` and the overlay with its includes expanded, exactly as `instructionsFor` joins them — and, against it, the digests every live session recorded when it started. A session running a role file that has been edited since is named by pid. Without a role it checks every live session against the role each one names. `--json`. |
...
| `mc test <repo> --full` | The repository's own whole suite on the branch `origin` calls its default, as fetched — one tree, no pull request, no selection. The only reading here that is about the code rather than about a change. `--json`. |
| `mc test nightly status` | Where the nightly's full run comes from, and what it found. The tick is a chore of the runner — every repository mc knows, once a day from the last tick, never while STOP is there — so the page says whether the runner is running (and when the next tick is due), stopping, or stopped; `start` and `stop` are gone and exit 2. A meter — what it finds refuses no merge and delays no round — and a tick that meets a running gate round skips it rather than queueing. Per repository: when the last measured run was, the commit, `N red of M`, and the oldest red test with the date its streak began; `--json` carries `runner`, `tick`, `interval_ms`, `log` and a `repos` object of one reading each. See [`docs/technical/mc-merge.md`](technical/mc-merge.md). |
...
| `mc gate [--base <ref>] [--json]` | The repository's own gate on the worktree the caller stands in — `npm run ci -- --base-ref origin/main` where `package.json` has a `ci` script, `npm test` otherwise — with the whole output written to a file (`$MC_SCRATCH` in a runner session, the system temp directory otherwise) and at most 2 KB printed: `GREEN`/`RED`, the counts, the failing lines with their locations, and the file's path (`gate-local.js`). A meter for a session before it pushes, not a door: `mc merge` measures again in its own tree. Exit 0 green, 1 red, 2 nothing to run. |
| `mc shot <dev\|prod> <target> [--el <css> \| --shell base\|immersive\|window] [--full] [--viewport <name\|WxH[@dpr]>] [--theme light\|dark] [--locale <code>] [--me] [--here] [--out <file.png>] [--open] [--json]` · `mc shot <dev\|prod> --list` · `mc shot login prod [--paste]` · `mc shot logout prod` | A PNG of the running app — a dev server started as `mc test dev` starts one, or production — and its path, alone, as the first line. memoro's `.mc/shot.json` names the capture script and the targets; mc hands it one request file (mode 0600, removed afterwards, the only place a token or cookie is written) and reads one result line. dev signs in as the seeded account through `/dev/login`; prod as the test account, or with `--me` as Martin from the session `mc shot login prod` stored in the keychain as `MEMORO_SHOT_SESSION`. Files go to `~/.memoro/mc/shots/` unless `--out`. See [`docs/technical/mc-shot.md`](technical/mc-shot.md). |
| `mc publish [--title …] [--body …] [--body-file <p>] [--json]` | memoro's `AGENTS.md` publication sequence as one call (`publish.js`): refuses on `main`, on a dirty tree, and when `origin/<branch>` holds a head not in this branch's history (never force); non-force `git push -u`; one `gh pr create` — or `gh pr edit` when the branch already has an open pull request and a title or body was given, never a second one — then one `gh pr view`. Title and body from the last commit unless given. Prints the number and URL, the head as GitHub holds it against the head pushed, and `mc merge <repo> <pr>` as the next line. |
...
| `mc brief <no args>` | The evaluation session: opens the brief session in the work root with the `brief` role and `Start the meeting.`, or resumes the one already there; `--new` starts a fresh one. Nothing is gathered for it — the role sends it to the page, `mc status <name>` and `mc step <project>`. `--collect` and `--offline` are gone (2026-09-19). |
| `mc helper` | The desk: a session in `~/mc/helper/` that turns a report of a bug or a rough edge into `~/mc/proposals/<date>-<x>.md`. `--intake` is the production eye — the daily digest, then one headless turn that proposes from it; `mc run` does this once a day. `--collect` writes only the digest. `--since`, `--limit`, `--threshold`, `--model`. |
| `mc log [--failures] [--limit <n>]` | What mc did and how it ended. Every invocation writes an `mc.start` and an `mc.end`; a start with no end is a command that died. `mc log <run>` joins the three files on one run id; `mc log --open` lists gate rounds that started and never ended, with the lease each left behind, and only reads. |
| `mc repo merge …` | **Gone** since 2026-08-26. Prints "mc repo merge is now mc merge" and exits 2. Not aliased. |
| `mc repo nightly …` | **Gone** since 2026-09-04. Prints "mc repo nightly is now mc test nightly" and exits 2. Not aliased: the scheduled full run belongs under the verb whose round it runs. |
...
| Command | Does |
|---|---|
| `mc vault <verb>` | The Memoro token vault: `setup`, `unlock`, `lock`, `status`, `set`, `get`, `list`, `rm`, `rotate`, `bind`, `bindings`, `import`, `scan`, `audit`, `adopt`, `hydrate`, `devices`, `recovery`, `recover`, `migrate`. Bare `mc vault` prints them with what each one takes. |

`mc vault` is the whole of `src/bin-mc.js`'s table. It is kept by the `mc-cut`
contract rather than by reachability, which is a distinction worth writing
down: `src/vault/credential-domain/` is not reached even by `mc vault`, only by
the broker world being removed, so it is kept deliberately alongside it.

---

## The session model, removed

`mc new` / `mc open` with a session home, a generation journal and a runtime
host was **old mc**. As of 2026-08-13 the local session store was empty: zero
sessions, and the 42 session homes, 429 pre-V1 records and their worktrees were
removed. The verbs went on 2026-08-30 — `mc new`, `mc open`, `mc resume`,
`mc restart`, `mc rename`, `mc cd`, `mc attach`, `mc end`, `mc delete`,
`mc cleanup`, `mc worktree`, `mc gc`, `mc storage` — and `mc doctor` and
`mc migrate`, the last two verbs that answered questions about that store,
went on 2026-09-03 with the rest of the verb list. Nothing forwards to any of
them; the router says the word is not a command.

There is no verb that lists sessions any more either — `mc list`,
`mc sessions list` and the old `mc status` board went with decision mc-3, and
`mc` is the page. `mc list` and `mc sessions` still answer, with where the page
went rather than with a list.
...
| `mc sessions watch \| stop \| remove` | Broker-owned session control. There is no broker and no session to control; `mc work stop <name>` stops what is running in a work area. |
| `mc setup`, `mc install-shell`, `mc auth`, `mc tool-auth`, `mc connections`, `mc github`, `mc coding-profile`, `mc dev`, `mc deps`, `mc cloud-session`, `mc cloud-runtime`, `mc security`, `mc migrate` | The capability table, removed 2026-09-03. Every one of them was a door into the session manager, the managed providers or the cloud runtimes — none is reached by the page or by any verb that survives it. `mc vault` is what is left of that table. |
