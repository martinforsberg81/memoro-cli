...
| `mc plan [<programme>]` | A planning session for one **programme**, with Martin at the terminal: a fresh foreground session in `~/mc/plan/<programme>/`, both repositories checked out on branch `plan/<programme>` from `origin/main`, carrying the `plan` role from `canon/roles/plan.md`. Its deliverable is a PR titled `Plan: <programme>` holding the programme document and a `PLAN.json` per project that can start, landed by the session itself with `mc merge <repo> <pr> --docs`. **Never a workarea**: `~/mc/plan/` holds no checkout, so neither `mc run`'s `workareas()` nor `mc status`'s `areasWithCheckout()` can see what is under it — a planning session and the runner share a file on `main` and nothing else. With no programme named it asks which: the programmes on `main` in either repository, the ones already being planned, or a new one to name. `--codex\|--claude`, `--model <m>`. |
