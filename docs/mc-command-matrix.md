...
| `mc watch main start \| stop \| status` | The main-watch (D-0190/D-0199): every 5 minutes it measures the base branch **per SHA** — a pass where `origin/main` has not moved costs one `git fetch` and no suite. A moved main already measured green by the gate (its baseline cache) is green for free; only a landing that bypassed the gate is actually run, under the suite right. It knocks pm the moment main goes red — the transition, not the state — naming the new red and the landings in the interval. `--repo <name>` on start (remembered like the guard's flags). |
