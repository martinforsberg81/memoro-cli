...
} from '../../../src/runtime/broker/c1-runner.js';
import { watchBrokerLivenessFixture } from '../../../src/runtime/broker/c1-lease-host.js';
...
test('C1 runner does not import vault code and pins the fixed lease host source', () => {
  const runner = readFileSync(new URL('../../../src/runtime/broker/c1-runner.js', import.meta.url), 'utf8');
  const host = readFileSync(new URL('../../../src/runtime/broker/c1-lease-host.js', import.meta.url));
  const closure = readFileSync(new URL('../../../src/runtime/broker/c1-source-closure.js', import.meta.url));
...
test('C1 source closure is verified before vault import and every credential pipe read/write', () => {
  const host = readFileSync(
    new URL('../../../src/runtime/broker/c1-lease-host.js', import.meta.url),
    'utf8',
  );
  const lease = readFileSync(
    new URL('../../../src/vault/engine/c1-claude-lease.js', import.meta.url),
    'utf8',
  );
  const child = readFileSync(
    new URL('../../../src/runtime/broker/c1-child.js', import.meta.url),
    'utf8',
  );
  assert.ok(host.indexOf('verifyInstalledC1SourceClosure()') >= 0);
  assert.ok(host.indexOf('verifyInstalledC1SourceClosure()')
    < host.indexOf("await import('../../vault/engine/c1-claude-lease.js')"));
...
test('C1 has exactly one detached group leader and waits for terminal group proof', () => {
  ...
  const child = readFileSync(new URL('../../../src/runtime/broker/c1-child.js', import.meta.url), 'utf8');
