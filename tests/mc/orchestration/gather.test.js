...
function makeFakeGit({
  ...
  return {
    ...
    fetch(_dir, ref) { calls.fetch.push(ref); return true; },
    ...
    push(_dir, branch) { calls.push.push(branch); return pushOk; },
  };
}
...
describe('mc gather — happy path', () => {
  ...
  test('merges phase PRs cleanly, pushes, opens summary PR', async () => {
    ...
    assert.deepEqual(git.calls.tryMerge, [
      'origin/fan/myplan/phase-1',
      'origin/fan/myplan/phase-2',
    ]);
    assert.deepEqual(git.calls.push, ['wip/myplan']);
  ...
  test('creates collection branch from origin/<from> when missing', async () => {
    ...
    assert.equal(git.calls.createCollectionBranch[0].fromRef, 'origin/main');
  });
  ...
  test('orders by phase number even when gh returns out of order', async () => {
    ...
    assert.deepEqual(git.calls.tryMerge, [
      'origin/fan/p/phase-1',
      'origin/fan/p/phase-2',
      'origin/fan/p/phase-3',
    ]);
  });

  test('defaults from_ref to main when registry has no phase entries', async () => {
...
describe('mc gather — conflict surface', () => {
  ...
  test('stops on first conflict, lists files + previous merges, exit 1', async () => {
    ...
    const git = makeFakeGit({
      mergeOutcomes: {
        'origin/fan/p/phase-2': { ok: false, conflicts: ['src/a.js', 'src/b.js'] },
      },
    });
    const registry = fakeRegistry([{ parent_plan: 'p', phase_n: 1, from_ref: 'main' }]);
  ...
  test('conflict surfaces on human-readable path too (no --json)', async () => {
    ...
    const git = makeFakeGit({
      mergeOutcomes: {
        'origin/fan/p/phase-1': { ok: false, conflicts: ['src/oops.js'] },
      },
    });
    const registry = fakeRegistry([]);
