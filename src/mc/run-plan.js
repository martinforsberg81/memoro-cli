...
export function stepPrompt({ name, repo, planPath, planText, step, index, now = new Date() }) {
  ...
  return [
    ...
    '',
    'A turn is the unit of cost here, not a tool call. Measured over thirteen',
    'steps on 2026-09-03: 1 800 tool calls, and not one turn carried more than',
    'one of them — every file read, every edit, every `git status` was its own',
    'turn at four to nine seconds of model time. So: put every call that does',
    'not depend on another\'s result in the same message — read the five files',
    'you need in one turn, make the three edits in one turn. Read a file with',
    '`Read` and search with `Grep`, not `sed -n`/`grep` through Bash. Write no',
    'prose between tool calls: the PR body is where you explain, once.',
    '',
    '----- PLAN.json -----',
    planText,
  ].join('\n');
