...
export function readSuiteLease({ root = mcHome(), now = Date.now() } = {}) {
  let raw = null;
...
export function claimSuiteLease({ errand, holder = currentHolder(), root = mcHome(), now = Date.now() } = {}) {
  const current = readSuiteLease({ root, now });
  ...
  log(root, `claim    holder=${holder.name}  errand="${lease.errand}"`);
  return { ok: true, already: false, lease: readSuiteLease({ root, now }) };
}

/** Give it back — or take it away, which is a different act and is logged. */
export function releaseSuiteLease({ holder = currentHolder(), force = false, root = mcHome(), now = Date.now() } = {}) {
  const current = readSuiteLease({ root, now });
